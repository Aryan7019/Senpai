...
export default function ATSScore({ content }: { content: string }) {
    ...
    return (
        <div className="gold-border-gradient rounded-xl shadow-xl p-4 flex items-center gap-4">
            <div className="relative w-12 h-12 shrink-0">
                ...
                <div className={`absolute inset-0 flex items-center justify-center text-xs font-bold ${getScoreColor()}`}>
            ...
            <button onClick={analyse} disabled={isLoading} className="text-xs font-semibold text-primary hover:text-primary/80 disabled:opacity-50 transition-colors whitespace-nowrap">
                {isLoading ? "Scoring..." : "Analyse"}
            </button>
        </div>
    );
}
